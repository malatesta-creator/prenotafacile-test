...
const AdminPanelV2: React.FC<AdminPanelProps> = ({ 
  ...
  useEffect(() => {
    if (editingId === 'new') {
      setFormData({ id: '', title: 'Nuovo Servizio', description: '', durationMinutes: 60, price: 100, imageUrl: `https://picsum.photos/800/600?random=${Date.now()}`, availability: { mode: 'always' } });
    } else if (editingId) {
  ...
  const handleSaveService = async () => {
    ...
    try {
        ...
        setEditingId(null);
    } catch (e) { alert("Errore salvataggio servizi"); console.error(e); } finally { setIsSaving(false); }
  };
  ...
  const handleSaveConfig = async () => {
      ...
      } catch (e) { alert("Errore salvataggio config"); console.error(e); } finally { setIsSaving(false); }
  };

  const handleBookingAction = async (id: string, status: BookingStatus) => {
    if (!window.confirm(status === BookingStatus.CONFIRMED ? "Confermare?" : "Cancellare?")) return;
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      <header className="bg-gray-900 text-white px-6 py-4 sticky top-0 z-50 flex justify-between items-center shadow-md">
        <div className="flex items-center gap-3"><h1 className="text-xl font-bold">{userRole === 'MASTER' ? `Master Control: ${targetClientName}` : `Admin: ${clientConfig.business_name}`}</h1></div>
      ...
      <main className="max-w-7xl mx-auto p-6 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {allClients.map(client => (
                        <div key={client.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                            ...
                            <button onClick={() => handleSelectClientToEdit(client)} className="w-full py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-bold">Modifica Setup ⚙️</button>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="space-y-4">
                {bookings.map(booking => (
                    <div key={booking.id} className="bg-white p-4 rounded shadow flex justify-between items-center">
                        <div><div className="font-bold">{booking.clientName}</div><div className="text-sm text-gray-600">{booking.service.title} - {booking.date}</div><div className="text-xs font-mono mt-1 px-2 py-0.5 bg-gray-100 inline-block rounded">{booking.status}</div></div>
                        {booking.status === 'PENDING' && (
                            <div className="flex gap-2">
                                <button onClick={() => handleBookingAction(booking.id, BookingStatus.CANCELLED)} className="px-3 py-1 bg-red-100 text-red-700 rounded text-sm font-bold">Rifiuta</button>
                                <button onClick={() => handleBookingAction(booking.id, BookingStatus.CONFIRMED)} className="px-3 py-1 bg-green-100 text-green-700 rounded text-sm font-bold">Conferma</button>
                            </div>
                        )}
                    </div>
                ))}
        ...
        {activeTab === 'services' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <button onClick={() => setEditingId('new')} className="mt-4 bg-indigo-600 text-white px-4 py-2 rounded font-bold w-full">+ Nuovo</button>
                </div>
                {editingId && (
                    <div className="bg-white p-6 rounded shadow h-fit">
                        ...
                        <input className="w-full border p-2 mb-2 rounded" type="number" placeholder="Prezzo" value={formData.price} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} />
                        ...
                        <button onClick={handleSaveService} disabled={isSaving} className="bg-indigo-600 text-white px-4 py-2 rounded w-full font-bold">Salva</button>
                    </div>
                )}
            </div>
        )}
        ...
        {activeTab === 'setup' && userRole === 'MASTER' && (
            <div className="max-w-xl mx-auto bg-white p-8 rounded shadow space-y-4">
                <h3 className="font-bold text-lg border-b pb-2">Setup Tecnico</h3>
                <input className="w-full border p-2 rounded" placeholder="Google API Key" value={configData.apiKey} onChange={e => setConfigData({...configData, apiKey: e.target.value})} />
                <input className="w-full border p-2 rounded" placeholder="Email Ponte" value={configData.calendarId} onChange={e => setConfigData({...configData, calendarId: e.target.value})} />
                <textarea className="w-full border p-2 rounded h-32 text-xs font-mono" placeholder="Service Account JSON" value={configData.serviceAccountJson} onChange={e => setConfigData({...configData, serviceAccountJson: e.target.value})} />
                <input className="w-full border p-2 rounded" placeholder="EmailJS Service ID" value={configData.emailServiceId} onChange={e => setConfigData({...configData, emailServiceId: e.target.value})} />
                <input className="w-full border p-2 rounded" placeholder="EmailJS Template ID" value={configData.emailTemplateId} onChange={e => setConfigData({...configData, emailTemplateId: e.target.value})} />
                <input className="w-full border p-2 rounded" placeholder="EmailJS Public Key" value={configData.emailPublicKey} onChange={e => setConfigData({...configData, emailPublicKey: e.target.value})} />
                <button onClick={handleSaveConfig} disabled={isSaving} className="bg-indigo-600 text-white w-full py-3 rounded font-bold mt-6 shadow-lg">Salva Configurazione</button>
            </div>
        )}
      </main>
    </div>
