...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const handleBookingAction = async (id: string, status: BookingStatus) => {
    ...
    setProcessingAction({ id, status });
    try { await onUpdateBookingStatus(id, status); } catch (e) { console.error(e); alert("Errore durante l'aggiornamento."); } finally { setProcessingAction(null); }
  }
  ...
  return (
    <div className="bg-gray-50 min-h-screen animate-fade-in flex flex-col">
      ...
      <main className="flex-grow max-w-7xl mx-auto p-6 w-full">
        {activeTab === 'bookings' && (
            <div className="space-y-6">
                <div className="flex justify-between items-end"><h2 className="text-2xl font-bold text-gray-800">Appuntamenti</h2><button onClick={handleResetData} className="text-xs text-red-500 hover:underline">Reset Demo</button></div>
                <div className="grid grid-cols-1 gap-4">
                    {sortedBookings.length === 0 && <p className="text-gray-500 italic">Nessuna prenotazione ricevuta.</p>}
                    {sortedBookings.map(booking => {
                        const isProcessing = processingAction?.id === booking.id;
        ...
        {activeTab === 'setup' && userRole === 'MASTER' && (
            <div className="max-w-2xl mx-auto space-y-8">
                <div><h2 className="text-2xl font-bold text-gray-800 mb-2">Setup Tecnico (Master)</h2><p className="text-gray-600">Configura le integrazioni esterne.</p></div>
                ...
                <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-200 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-sm font-bold text-gray-800 mb-2">Service ID</label><input type="text" className="w-full px-4 py-3 border border-gray-300 rounded-lg font-mono text-sm" placeholder="service_xyz" value={configData.emailServiceId} onChange={(e) => setConfigData({...configData, emailServiceId: e.target.value})} /></div>
                        <div><label className="block text-sm font-bold text-gray-800 mb-2">Template ID</label><input type="text" className="w-full px-4 py-3 border border-gray-300 rounded-lg font-mono text-sm" placeholder="template_abc" value={configData.emailTemplateId} onChange={(e) => setConfigData({...configData, emailTemplateId: e.target.value})} /></div>
                    </div>
                    <div><label className="block text-sm font-bold text-gray-800 mb-2">Public Key</label><input type="text" className="w-full px-4 py-3 border border-gray-300 rounded-lg font-mono text-sm" placeholder="W2k-Js..." value={configData.emailPublicKey} onChange={(e) => setConfigData({...configData, emailPublicKey: e.target.value})} /></div>

                    <div className="pt-4 flex items-center justify-between"><span className={`text-green-600 font-bold transition-opacity ${configSaved ? 'opacity-100' : 'opacity-0'}`}>✓ Salvato!</span><button onClick={handleSaveConfig} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold transition-colors shadow-md">Salva Configurazione</button></div>
                </div>
...
export default AdminPanel;   <div className="flex justify-end gap-3 pt-4"><button onClick={() => setEditingId(null)} className="px-4 py-2 text-gray-600">Annulla</button><button onClick={handleSaveService} className="px-4 py-2 bg-indigo-600 text-white rounded font-bold">Salva</button></div>
                            </div>
                        </div>
                    )}
                </div>
