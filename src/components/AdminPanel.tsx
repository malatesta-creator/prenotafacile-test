      ...
      <main className="max-w-7xl mx-auto p-6 w-full">
        ...
        {activeTab === 'setup' && userRole === 'MASTER' && (
            <div className="max-w-xl mx-auto bg-white p-8 rounded shadow space-y-4">
                ...
                <label className="block text-sm font-bold mt-4">Google Cloud API Key (Lettura)</label>
                ...
                <label className="block text-sm font-bold mt-4">ID Calendario Google (Principale)</label>
                <p className="text-xs text-gray-500">Normalmente è l'email del proprietario. Lascia vuoto per usare {clientConfig.email_owner}, oppure scrivi un'email diversa per forzare la scrittura altrove.</p>
                <input className="w-full border p-2 rounded" placeholder={clientConfig.email_owner} value={configData.calendarId} onChange={e => setConfigData({...configData, calendarId: e.target.value})} />
